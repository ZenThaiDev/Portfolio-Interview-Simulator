...
class InterviewGUI:
    ...
    async def async_select_file(self, file_path):
        ...
        try:
            validation_result = await self.simulator.read_portfolio(file_path)
            self.progress_bar.show_progress(False)

            self.logger.info(f"Validation result: {validation_result}")

            # Display validation result in chat
            if validation_result["valid"]:
                self.display_message(
                    "System",
                    f"Portfolio validation successful: {validation_result['message']}",
                )
                self.status_bar.update_status("Portfolio is valid")
                ...
                self.simulator.portfolio_data = validation_result["data"]
            else:
                self.display_message(
                    "System",
                    f"Portfolio validation failed: {validation_result['message']}",
                )
                self.status_bar.update_status("Portfolio is invalid", is_error=True)
                self.start_button.configure(state="disabled")
                self.simulator.portfolio_data = None
        except Exception as e:
            self.logger.error(f"Error processing file: {e}")
            self.status_bar.update_status(f"Error processing file: {str(e)}", is_error=True)
            self.progress_bar.show_progress(False)
    ...
    async def async_start_interview(self):
        ...
        try:
            response_raw = await self.simulator.start_interview()
            self.logger.debug(f"Response: {response_raw}")

            try:
                response_data = json.loads(response_raw)

                if response_data["data"]["message_type"] == "question":
                    self.question_progress.update_progress(1)
                    
                # Get text for TTS
                message_data = response_data["data"]
                tts_text = message_data["message"]
            except (json.JSONDecodeError, KeyError) as e:
                self.logger.error(f"Error parsing response: {e}")
                tts_text = response_raw

            self.display_message("Interviewer", response_raw)
            self.status_bar.update_status("Interview started")
            ...
            await self.audio_manager.play_question(
                tts_text,
            ...
            self.status_bar.update_status("Error starting interview", is_error=True)
    ...
    async def async_submit_answer(self, answer):
        ...
        try:
            self.display_message("You", answer)
            self.answer_entry.delete("1.0", "end")
            response = await self.simulator.submit_answer(answer)

            self.logger.info(f"Response: {response}")

            try:
                response_data = json.loads(response)
                self.display_message("Interviewer", response)

                # Get text for TTS based on response type
                is_final = False
                if (
                    "final_evaluation" in response_data
                    and response_data["final_evaluation"]
                ):
                    tts_text = response_data["final_evaluation"]
                    is_final = True
                    self.submit_button.configure(state="disabled")
                    self.answer_entry.configure(state="disabled")
                    self.status_bar.update_status("Interview completed")
                    self.question_progress.update_progress(
                    ...
                    # Only increment question count if it's not a follow-up
                    if message_data["message_type"] == "question":
                        self.question_progress.update_progress(
                            self.simulator.config.question_count
                        )

                    self.status_bar.update_status("Ready for your next answer")
                ...
                self.enable_input_controls(is_final)

            except (json.JSONDecodeError, KeyError) as e:
                # Fallback to old behavior if JSON parsing fails, just in case
                self.logger.error(f"Error parsing response: {e}")
                self.display_message("Interviewer", response)

                # For non-JSON responses, try to extract just the evaluation text
                tts_text = (
                    response.split("\n\nScores:")[0]
                    if "\n\nScores:" in response
                    else response
                ...
                self.status_bar.update_status("Ready for your next answer")
                self.question_progress.update_progress(self.simulator.config.question_count)
            ...
            self.status_bar.update_status("Error processing answer", is_error=True)
            self.enable_input_controls()
        finally:
            self.progress_bar.show_progress(False)
    ...
    def display_message(self, sender, message):
        ...
        try:
            if sender == "Interviewer":
                ...
                # Handle the message based on the data type
                if "data" in response:
                ...
                if "final_evaluation" in response and response["final_evaluation"]:
                    display_text = response["final_evaluation"]

                    if "scores" in response:
                        scores = response["scores"]
                        score_text = "\n\nScores:"
                        score_text += f"\n- Clarity and Communication: {scores['clarity_and_communication']}/25"
                        score_text += f"\n- Relevance and Content: {scores['relevance_and_content']}/25"
                        score_text += f"\n- Critical Thinking: {scores['critical_thinking']}/25"
                        score_text += f"\n- Overall Impact: {scores['overall_impact']}/25"
                        total = (
                            scores["clarity_and_communication"]
                            + scores["relevance_and_content"]
        ...
        self.chat_area.add_message(sender, display_text, timestamp, message_type)
        self.logger.debug(f"Message from {sender}: {display_text[:100]}...")
